...
def test_scoped_forms_set_type_flags(event, image_uploads):
    sponsor_form = ExhibitorInfoForm(
        data={"name_0": "Acme Sponsor"}, files=image_uploads(), event=event, partner_type="sponsor"
    )
    ...
    exhibitor_form = ExhibitorInfoForm(
        data={"name_0": "Acme Exhibitor"}, files=image_uploads(), event=event, partner_type="exhibitor"
    )
