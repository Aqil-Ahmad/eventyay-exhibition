...
def _bulk_view(event, partner_type="exhibitor", *, data=None, selected=None):
    data = dict(data or {})
...
def test_bulk_send_only_targets_its_own_partner_type(voucher_event):
    with scopes_disabled():
        ...
        view, request = _bulk_view(
            voucher_event, partner_type="sponsor", data={"confirmed": "1"}, selected=[booth, gold]
        )
...
@pytest.mark.django_db
def test_bulk_send_only_reaches_the_selected_partners(voucher_event):
    with scopes_disabled():
        ExhibitorSettings.objects.create(event=voucher_event, voucher_default_count=1, voucher_pool_tag=POOL)
        _pool(voucher_event, 4)
